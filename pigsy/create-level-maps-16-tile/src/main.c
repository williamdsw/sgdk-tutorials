...
int main()
{
    ...
    PAL_setPalette(PAL1, dog_img.palette->data, DMA);

    VDP_loadTileSet(&bg_tileset, ind, DMA);
    level_map = MAP_create(&bg_map, BG_A, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, ind));
    ind += bg_tileset.numTile;

    MAP_scrollTo(level_map, 0, 0);

    spr_dog = SPR_addSprite(&dog_img, fix32ToInt(dog_x), fix32ToInt(dog_y), TILE_ATTR(PAL1, FALSE, FALSE, FALSE));
