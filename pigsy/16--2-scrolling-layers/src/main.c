...
int main()
{
    ...
    PAL_setPalette(PAL0, background_img.palette->data, DMA);
    PAL_setPalette(PAL1, foreground_pal.data, DMA);
    PAL_setPalette(PAL2, dog_img.palette->data, DMA);

    VDP_loadTileSet(&foreground_tileset, ind, DMA);
    level_map = MAP_create(&foreground_map, BG_A, TILE_ATTR_FULL(PAL1, FALSE, FALSE, FALSE, ind));
    ind += foreground_tileset.numTile;

    VDP_drawImageEx(BG_B, &background_img, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, ind), 0, 0, FALSE, TRUE);
    ind += background_img.tileset->numTile;
...
static void handleCamera()
{
    ...
    if ((current_camera_x != new_camera_x) || (current_camera_y != new_camera_y))
    {
        ...
        // Megadrive CPU is not good with multiplication and division, use bit shift
        // b_hscroll = (0 - new_camera_x) / 8;
        b_hscroll = (0 - new_camera_x) >> 3;
        b_vscroll = new_camera_y >> 1;

        if (b_vscroll < 0)
        {
            b_vscroll = 0;
        }
        else if (b_vscroll > 32)
        {
            b_vscroll = 32;
        }
